...
#define LMPLAYER_PLUGIN_MANAGER_GET_PRIVATE(object)(G_TYPE_INSTANCE_GET_PRIVATE ((object), LMPLAYER_TYPE_PLUGIN_MANAGER, LmplayerPluginManagerPrivate))

struct _LmplayerPluginManagerPrivate
{
	GList		*plugins;
	GtkWidget	*tree;
...
G_DEFINE_TYPE(LmplayerPluginManager, lmplayer_plugin_manager, GTK_TYPE_VBOX)

static void lmplayer_plugin_manager_finalize (GObject *o);
static LmplayerPluginInfo *plugin_manager_get_selected_plugin (LmplayerPluginManager *pm);
static void plugin_manager_toggle_active (GtkTreeIter *iter, GtkTreeModel *model, LmplayerPluginManager *pm);
static void plugin_manager_toggle_all (LmplayerPluginManager *pm);

/* Callback functions for GtkBuilder */
G_MODULE_EXPORT void configure_button_cb (GtkWidget *button, LmplayerPluginManager *pm);

static void
lmplayer_plugin_manager_class_init (LmplayerPluginManagerClass *klass)
{
	GObjectClass *object_class = G_OBJECT_CLASS (klass);

	object_class->finalize = lmplayer_plugin_manager_finalize;

	g_type_class_add_private (object_class, sizeof (LmplayerPluginManagerPrivate));
}

void
configure_button_cb (GtkWidget          *button,
		     LmplayerPluginManager *pm)
{
	...
	lmplayer_plugins_engine_configure_plugin (info, toplevel);
}
			     ...
			     gpointer           data)
{
	...
	g_return_if_fail (lmplayer_plugins_engine_get_plugin_name (info) != NULL);

	g_object_set (G_OBJECT (cell),
		      "text",
		      lmplayer_plugins_engine_get_plugin_name (info),
		      NULL);
}
		   ...
		   gpointer     data)
{
	...
	gtk_label_set_text (GTK_LABEL (pm->priv->description_text),
			    lmplayer_plugins_engine_get_plugin_description (info));
	gtk_label_set_text (GTK_LABEL (pm->priv->copyright_text),
			    lmplayer_plugins_engine_get_plugin_copyright (info));
	gtk_label_set_text (GTK_LABEL (pm->priv->site_text),
			    lmplayer_plugins_engine_get_plugin_website (info));

	string = g_strjoinv ("\n", (gchar**)lmplayer_plugins_engine_get_plugin_authors (info));
	gtk_label_set_text (GTK_LABEL (pm->priv->authors_text), string);
	g_free (string);

	icon = lmplayer_plugins_engine_get_plugin_icon (info);
	if (icon != NULL) {
	...
	gtk_widget_set_sensitive (GTK_WIDGET (pm->priv->configure_button),
				  (info != NULL) &&
				   lmplayer_plugins_engine_plugin_is_configurable (info));
}
...
column_clicked_cb (GtkTreeViewColumn *tree_column,
		   gpointer           data)
{
	LmplayerPluginManager *pm = LMPLAYER_PLUGIN_MANAGER (data);

	plugin_manager_toggle_all (pm);
}

static void
plugin_manager_populate_lists (LmplayerPluginManager *pm)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
	GList *p;
	...
	for (p = pm->priv->plugins; p != NULL; p = g_list_next (p)) {
		LmplayerPluginInfo *info;
		info = (LmplayerPluginInfo *)p->data;
		...
		gtk_list_store_set (GTK_LIST_STORE (pm->priv->plugin_model), &iter,
				    ACTIVE_COLUMN, lmplayer_plugins_engine_plugin_is_active (info),
				    VISIBLE_COLUMN, lmplayer_plugins_engine_plugin_is_visible (info),
				    INFO_COLUMN, info,
				    -1);
	}
			   ...
			   LmplayerPluginManager *pm)
{
	...
	if (active) {
		/* activate the plugin */
		if (!lmplayer_plugins_engine_activate_plugin (info)) {
			active ^= 1;
		}
	} else {
		/* deactivate the plugin */
		if (!lmplayer_plugins_engine_deactivate_plugin (info)) {
			active ^= 1;
		}
	}
	...
	gtk_list_store_set (GTK_LIST_STORE (pm->priv->plugin_model),
			    &child_iter,
			    ACTIVE_COLUMN,
			    lmplayer_plugins_engine_plugin_is_active (info),
			    -1);
...
plugin_manager_get_selected_plugin (LmplayerPluginManager *pm)
{
	LmplayerPluginInfo *info = NULL;
	GtkTreeModel *model;
	GtkTreeIter iter;
	GtkTreeSelection *selection;
...
static void
plugin_manager_toggle_all (LmplayerPluginManager *pm)
{
	GtkTreeModel *model;
	GtkTreeIter iter;
		...
		gpointer      data)
{
	...
	normalized_string = g_utf8_normalize (lmplayer_plugins_engine_get_plugin_name (info), -1, G_NORMALIZE_ALL);
...
static void
plugin_manager_construct_tree (LmplayerPluginManager *pm)
{
	GtkTreeViewColumn *column;
	GtkCellRenderer *cell;
	GtkTreeModel *filter;
...
{
	LmplayerPluginInfo *lhs = (LmplayerPluginInfo*)a;
	LmplayerPluginInfo *rhs = (LmplayerPluginInfo*)b;
	return strcmp (lmplayer_plugins_engine_get_plugin_name (lhs),
		       lmplayer_plugins_engine_get_plugin_name (rhs));
}

static void
lmplayer_plugin_manager_init (LmplayerPluginManager *pm)
{
	GtkBuilder *xml;
	GtkWidget *plugins_window;

	pm->priv = LMPLAYER_PLUGIN_MANAGER_GET_PRIVATE (pm);

	xml = lmplayer_interface_load ("plugins.ui", TRUE, NULL, pm);
	...
	pm->priv->plugins = lmplayer_plugins_engine_get_plugins_list ();
...
GtkWidget *
lmplayer_plugin_manager_new (void)
{
	return g_object_new (LMPLAYER_TYPE_PLUGIN_MANAGER, 0);
}

static void
lmplayer_plugin_manager_finalize (GObject *o)
{
	LmplayerPluginManager *pm = LMPLAYER_PLUGIN_MANAGER (o);

	g_list_free (pm->priv->plugins);

	G_OBJECT_CLASS (lmplayer_plugin_manager_parent_class)->finalize (o);
}
