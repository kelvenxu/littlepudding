		...
		GtkWindow *parent)
{
	...
	if (reason == NULL)
		g_warning ("lmplayer_action_error called with reason == NULL");
	...
	lmplayer_interface_set_transient_for (GTK_WINDOW (error_dialog),
				GTK_WINDOW (parent));
...
void
lmplayer_interface_error (const char *title, const char *reason,
		GtkWindow *parent)
{
	GtkWidget *error_dialog;
...
void
lmplayer_interface_error_blocking (const char *title, const char *reason,
		GtkWindow *parent)
{
	GtkWidget *error_dialog;
...
GtkBuilder *
lmplayer_interface_load (const char *name, gboolean fatal, GtkWindow *parent, gpointer user_data)
{
...
GdkPixbuf*
lmplayer_interface_load_pixbuf (const char *name)
{
	GdkPixbuf *pix;
	char *filename;
...
char *
lmplayer_interface_get_full_path (const char *name)
{
	char *filename;
...
void
lmplayer_interface_set_transient_for (GtkWindow *window, GtkWindow *parent)
{
	if (GTK_IS_PLUG (parent)) {
		GdkWindow *toplevel;

		gtk_widget_realize (GTK_WIDGET (window));
		toplevel = lmplayer_gtk_plug_get_toplevel (GTK_PLUG (parent));
		if (toplevel != NULL) {
...
char *
lmplayer_interface_get_license (void)
{
	const char *license[] = {
		N_("Lmplayer is free software; you can redistribute it and/or modify "
		   "it under the terms of the GNU General Public License as published by "
		   "the Free Software Foundation; either version 2 of the License, or "
		   "(at your option) any later version."),
		N_("Lmplayer is distributed in the hope that it will be useful, "
		   "but WITHOUT ANY WARRANTY; without even the implied warranty of "
		   "MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the "
		   "GNU General Public License for more details."),
		N_("You should have received a copy of the GNU General Public License "
		   "along with Lmplayer; if not, write to the Free Software Foundation, Inc., "
		   "59 Temple Place, Suite 330, Boston, MA  02111-1307  USA"),
		N_("Lmplayer contains an exception to allow the use of proprietary "
		   "GStreamer plugins.")
	};
	return g_strjoin ("\n\n",
			  _(license[0]),
...
void
lmplayer_interface_boldify_label (GtkBuilder *builder, const char *name)
{
	GtkLabel *widget;
	gchar *str_final;
