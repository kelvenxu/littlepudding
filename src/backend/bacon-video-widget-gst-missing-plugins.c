...
GST_DEBUG_CATEGORY_EXTERN (_lmplayer_gst_debug_cat);
...
LmplayerCodecInstallContext;

#ifdef GDK_WINDOWING_X11
/* Adapted from lmplayer-interface.c */
...
static void
bacon_video_widget_gst_codec_install_context_free (LmplayerCodecInstallContext *ctx)
{
...
{
	LmplayerCodecInstallContext *ctx = (LmplayerCodecInstallContext *) user_data;
	gchar **p;
						 ...
						 gpointer user_data)
{
	...
	LmplayerCodecInstallContext *ctx;
	...
	ctx = g_new0 (LmplayerCodecInstallContext, 1);
	 ...
	 * be able to use lmplayer's controls while the wizard is running */
	if (playing)
