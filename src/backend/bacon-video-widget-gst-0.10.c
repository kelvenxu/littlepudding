 ...
 *
 * The Lmplayer project hereby grant permission for non-gpl compatible GStreamer
 * plugins to be used and distributed together with GStreamer and Lmplayer. This
 * permission is above and beyond the permissions granted by the GPL license
 * Lmplayer is covered by.
 *
...
GST_DEBUG_CATEGORY (_lmplayer_gst_debug_cat);
#define GST_CAT_DEFAULT _lmplayer_gst_debug_cat
...
bacon_video_widget_realize (GtkWidget * widget)
{
  ...
  lmplayer_widget_set_preferred_size (widget, w, h);
...
bvw_handle_application_message (BaconVideoWidget *bvw, GstMessage *msg)
{
  ...
  if (strcmp (msg_name, "notify-streaminfo") == 0) {
	...
	&& !bvw->priv->window_resized) {
      ...
      shrink_toplevel (bvw);
      get_media_size (bvw, &w, &h);
      lmplayer_widget_set_preferred_size (GTK_WIDGET (bvw), w, h);
    } else {
 ...
 * from the bus message callback (via EOS => lmplayer => close => wait for READY)
...
timed_out:
  /* it's taking a long time to open -- just tell lmplayer it was ok, this allows
    ...
    const gchar * mrl, const gchar *subtitle_uri, GError ** error)
{
  ...
  if (ret) {
    ...
    if (bvw_check_missing_plugins_error (bvw, err_msg)) {
      /* lmplayer will try to start playing, so ignore all messages on the bus */
                                    ...
                                    gboolean show_cursor)
{
  ...
  if (show_cursor == FALSE) {
    lmplayer_gdk_window_set_invisible_cursor (GTK_WIDGET (bvw)->window);
  } else {
...
bacon_video_widget_set_scale_ratio (BaconVideoWidget * bvw, gfloat ratio)
{
  ...
  if (ratio == 0.0) {
    if (lmplayer_ratio_fits_screen (bvw->priv->video_window, w, h, 2.0))
      ratio = 2.0;
    else if (lmplayer_ratio_fits_screen (bvw->priv->video_window, w, h, 1.0))
      ratio = 1.0;
    else if (lmplayer_ratio_fits_screen (bvw->priv->video_window, w, h, 0.5))
      ratio = 0.5;
    else
      return;
  } else {
    if (!lmplayer_ratio_fits_screen (bvw->priv->video_window, w, h, ratio)) {
          ...
          gpointer data)
{
  ...
  if (!strcmp (entry->key, "/apps/lmplayer/network-buffer-threshold")) {
    g_object_set (bvw->priv->play, "queue-threshold",
        (guint64) (GST_SECOND * gconf_value_get_float (entry->value)), NULL);
  } else if (!strcmp (entry->key, "/apps/lmplayer/buffer-size")) {
                        ...
                        BvwUseType type, GError ** err)
{
  ...
  if (_lmplayer_gst_debug_cat == NULL) {
    GST_DEBUG_CATEGORY_INIT (_lmplayer_gst_debug_cat, "lmplayer", 0,
        "Lmplayer GStreamer Backend");
  }
#endif
  ...
  gconf_client_notify_add (bvw->priv->gc, "/apps/lmplayer",
