...
void
lmplayer_gdk_window_set_invisible_cursor (GdkWindow *window)
{
...
void
lmplayer_gdk_window_set_waiting_cursor (GdkWindow *window)
{
	GdkCursor *cursor;
...
gboolean
lmplayer_display_is_local (void)
{
...
char *
lmplayer_time_to_string (gint64 msecs)
{
...
char *
lmplayer_time_to_string_text (gint64 msecs)
{
...
typedef struct _LmplayerPrefSize {
  gint width, height;
  gulong sig_id;
} LmplayerPrefSize;
...
void
lmplayer_widget_set_preferred_size (GtkWidget *widget, gint width,
				 gint height)
{
  LmplayerPrefSize *size = g_new (LmplayerPrefSize, 1);
...
gboolean
lmplayer_ratio_fits_screen (GdkWindow *video_window, int video_width,
