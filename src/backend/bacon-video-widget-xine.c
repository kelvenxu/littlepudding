 ...
 *
 * $Id: bacon-video-widget-xine.c 5613 2008-08-28 14:21:10Z hadess $
 *
 ...
 * path is something like ~/.config/lmplayer/xine_config */
#define CONFIG_FILE "lmplayer"G_DIR_SEPARATOR_S"xine_config"
...
load_video_out_driver (BaconVideoWidget *bvw, BvwUseType type)
{
	 ...
	 * on a local display */
	if (lmplayer_display_is_local () == FALSE ||
...
bacon_video_widget_set_scale_ratio (BaconVideoWidget *bvw, gfloat ratio)
{
	...
	if (ratio == 0)
	{
		if (lmplayer_ratio_fits_screen (widget->window, bvw->priv->video_width, bvw->priv->video_height, 2) != FALSE) {
			ratio = 2;
		} else if (lmplayer_ratio_fits_screen (widget->window, bvw->priv->video_width, bvw->priv->video_height, 1) != FALSE) {
			ratio = 1;
		} else if (lmplayer_ratio_fits_screen (widget->window, bvw->priv->video_width, bvw->priv->video_height, 0.5) != FALSE) {
			ratio = 0.5;
		} else {
			return;
		}
		 ...
		 * us some room */
		if (lmplayer_ratio_fits_screen (widget->window, bvw->priv->video_width, bvw->priv->video_height, ratio) == FALSE)
			return;
	}
	...
	lmplayer_widget_set_preferred_size (toplevel, new_w, new_h);
}
