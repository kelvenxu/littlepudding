...
gboolean
lmplayer_playing_dvd (const char *uri)
{
	if (uri == NULL)
		return FALSE;
...
lmplayer_dot_dir (void)
{
	...
	lmplayer_dir = g_build_filename (g_get_user_config_dir (),
				      "lmplayer",
				      NULL);
...
static GMount *
lmplayer_get_mount_for_uri (const char *path)
{
	GMount *mount;
	GFile *file;
...
static char *
lmplayer_get_mountpoint_for_dvd (const char *uri)
{
...
static char *
lmplayer_get_mountpoint_for_vcd (const char *uri)
{
	return NULL;
}

GMount *
lmplayer_get_mount_for_media (const char *uri)
{
	GMount *ret;
	char *mount_path;
	...
	if (g_str_has_prefix (uri, "dvd://") != FALSE)
		mount_path = lmplayer_get_mountpoint_for_dvd (uri);
	else if (g_str_has_prefix (uri, "vcd:") != FALSE)
		mount_path = lmplayer_get_mountpoint_for_vcd (uri);
	...
	ret = lmplayer_get_mount_for_uri (mount_path);
	g_free (mount_path);
...
static gboolean
lmplayer_is_special_mrl (const char *uri)
{
	GMount *mount;
	...
	mount = lmplayer_get_mount_for_media (uri);
	if (mount != NULL)
		g_object_unref (mount);
...
gboolean
lmplayer_is_block_device (const char *uri)
{
	struct stat buf;
	char *local;
...
char *
lmplayer_create_full_path (const char *path)
{
	GFile *file;
	char *retval;
	...
	if (strstr (path, "://") != NULL)
		return NULL;
	if (lmplayer_is_special_mrl (path) != FALSE)
		return NULL;
...
static void
lmplayer_action_on_unmount (GVolumeMonitor *volume_monitor,
			 GMount *mount,
...
void
lmplayer_setup_file_monitoring (LmplayerObject *lmplayer)
{
	lmplayer->monitor = g_volume_monitor_get ();

	g_signal_connect (G_OBJECT (lmplayer->monitor),
			  "mount-pre-unmount",
			  G_CALLBACK (lmplayer_action_on_unmount),
			  lmplayer);
}
...
lmplayer_uri_exists (const char *uri)
{
...
static char *
lmplayer_uri_get_subtitle_for_uri (const char *uri)
{
	...
	for (i = 0; i < G_N_ELEMENTS (subtitle_ext) ; i++) {
		...
		if (lmplayer_uri_exists (subtitle))
			return subtitle;
...
static char *
lmplayer_uri_get_subtitle_in_subdir (GFile *file, const char *subdir)
{
	...
	subtitle = lmplayer_uri_get_subtitle_for_uri (full_path_str);
...
char *
lmplayer_uri_get_subtitle_uri (const char *uri)
{
	GFile *file;
	char *subtitle;
	...
	subtitle = lmplayer_uri_get_subtitle_for_uri (uri);
	if (subtitle != NULL) {
		g_object_unref (file);
		return subtitle;
	}

	subtitle = lmplayer_uri_get_subtitle_in_subdir (file, "subtitles");
	g_object_unref (file);
...
char *
lmplayer_uri_escape_for_display (const char *uri)
{
	GFile *file;
	char *disp;
...
void
lmplayer_setup_file_filters (void)
{
	guint i;
...
void
lmplayer_destroy_file_filters (void)
{
	if (filter_all != NULL) 
	{
...
static void
lmplayer_add_default_dirs (GtkFileChooser *dialog)
{
	guint i;
...
void
lmplayer_add_pictures_dir (GtkWidget *chooser)
{
	const char *dir;
...
char *
lmplayer_add_subtitle (GtkWindow *parent, const char *path)
{
	GtkWidget *fs;
	GConfClient *conf;
	char *new_path;
	...
	if (path != NULL) {
		...
		new_path = gconf_client_get_string (conf, "/apps/lmplayer/open_path", NULL);
...
GSList *
lmplayer_add_files (GtkWindow *parent, const char *path)
{
	GtkWidget *fs;
	int response;
	GSList *filenames;
	...
	if (mrl != NULL) {
		...
		gconf_client_set_string (conf, "/apps/lmplayer/open_path",
				new_path, NULL);
		g_free (new_path);
	}
